const { badRequest, fetchFinnhub, fetchFinnhubQuote, fetchTwelveQuote, getSymbol, sendJson } = require('./_utils');

module.exports = async function handler(req, res) {
  const symbol = getSymbol(req);
  if (!symbol) return badRequest(res, 'Missing symbol.');
  try {
    const [twelveQuote, finnhubQuote, recommendation, profile] = await Promise.all([
    ...
    let quote = twelveQuote || finnhubQuote || { c: null, dp: null };
